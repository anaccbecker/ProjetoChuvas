...
###### Leitura dos dados ######
contagem = pd.read_csv("csv/indices_periodo_completo.csv")
...
cb = plt.colorbar(ax.get_children()[1], ax=ax, shrink=0.7)
cb.ax.tick_params(labelsize=6)
plt.savefig('img/[Geo] Índices/FAR.jpg', dpi = 400)
...
plt.savefig('img/[Geo] Índices/all.jpg', dpi = 400)
...
# for i, column in enumerate(['ACC', 'POD', 'BIAS', 'HSS']):
#     ax = axs[i]
#     ax = parana.plot(color='white', edgecolor='gray', linewidth = 0.5, ax=ax)

#     results_geo.plot(column=column, ax = ax, cmap=cmap)

#     for i in range(0,len(results)):
#         ax.annotate(f'{results.nome[i]} \n {results[column][i]*100:.2f}%', 
#                      xy=(results.longitude[i], results.latitude[i]), 
#                      xycoords='data',
...
# cb = plt.colorbar(ax.get_children()[1], cax=cbar_ax, shrink=0.7)
# cb.ax.tick_params(labelsize=6)

# plt.savefig('img/[Geo] Indices/all.jpg', dpi = 400)